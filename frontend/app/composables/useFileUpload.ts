...
export function useFileUploadWithStatus(chatId?: string) {
  ...
  const upload = useUpload((chatId: string) => `/api/upload/${chatId}`, { method: 'PUT' })

  async function uploadFiles(newFiles: File[], uploadChatId?: string) {
    const targetChatId = uploadChatId || chatId
    if (!loggedIn.value || !targetChatId) {
      // 如果没有chatId，仍然添加文件到列表但不上传
    ...
    const uploadPromises = filesWithStatus.map(async (fileWithStatus) => {
      ...
      try {
        ...
        const response = await upload(targetChatId, input) as BlobResult | BlobResult[] | undefined
  ...
  const { dropzoneRef, isDragging } = useFileUpload({
    ...
    multiple: true,
    onUpdate: (files) => uploadFiles(files)
  })
